...
# ---------------------------
    ...
    database = 'TEST_FCB'
    schema= 'DBT_ATIWARI'
    target_table = f"{database}.{schema}.DBT_TEST_RESULTS"
    dbt_path='models/FCB/raw_vault/sats'
    target_path='target'
    ...
    with open(rf'{target_path}/run_results.json', 'r') as input_file:
        ...
        my_tables = []
    with open(rf'{dbt_path}/satellite.yml') as f:
    ...
    def run_query(conn, query):
            ...
            cursor.close()
            insert_values = f"""insert into {target_table}
                                test_name, 
                                model_name,
                                column_names, 
                                test_status, 
