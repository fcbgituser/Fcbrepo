...
{%- set joins = [
  {
    'src_schema': 'tpch_sample', 'src_model': 'LINEITEM', 'alias': 'a', 'type': 'left',
    'on': [
      ['a.L_ORDERKEY', 'b.O_ORDERKEY']
    ]
  },
  {
    'src_schema': 'tpch_sample', 'src_model': 'CUSTOMER', 'alias': 'c', 'type': 'left',
    'on': [
      ['b.O_CUSTKEY', 'c.C_CUSTKEY']
    ]
  },
  {
    'src_schema': 'tpch_sample', 'src_model': 'NATION', 'alias': 'd', 'type': 'left',
    'on': [
      ['c.C_NATIONKEY', 'd.N_NATIONKEY']
    ]
  },
  {
    'src_schema': 'tpch_sample', 'src_model': 'REGION', 'alias': 'e', 'type': 'left',
    'on': [
      ['d.N_REGIONKEY', 'e.R_REGIONKEY']
    ]
  },
  {
    'src_schema': 'tpch_sample', 'src_model': 'PART', 'alias': 'g', 'type': 'left',
    'on': [
      ['a.L_PARTKEY', 'g.P_PARTKEY']
    ]
  },
  {
    'src_schema': 'tpch_sample', 'src_model': 'SUPPLIER', 'alias': 'h', 'type': 'left',
    'on': [
      ['a.L_SUPPKEY', 'h.S_SUPPKEY']
    ]
  },
  {
    'src_schema': 'tpch_sample', 'src_model': 'NATION', 'alias': 'j', 'type': 'left',
    'on': [
      ['h.S_NATIONKEY', 'j.N_NATIONKEY']
    ]
  },
  {
    'src_schema': 'tpch_sample', 'src_model': 'REGION', 'alias': 'k', 'type': 'left',
    'on': [
      ['j.N_REGIONKEY', 'k.R_REGIONKEY']
    ]
  }
] -%}

{# Where clause uses the same var('load_date') you had #}
{%- set where_clause = "b.O_ORDERDATE = TO_DATE('" ~ var('load_date') ~ "')" -%}

{{ stage_table_generation_select('tpch_sample', 'ORDERS',
